...
export function useNdviRequest() {
  ...
  const request = useCallback(async (bbox: BBox3857, date: string) => {
    setState({ status: 'loading' })
    try {
      const { blob, provider } = await fetchNdvi(bbox, date)
      ...
      else if (code === '404') message = 'Снимок не найден: нет сцены с облачностью < 20% для выбранной даты и области. Попробуйте изменить дату ±2–3 недели.'
